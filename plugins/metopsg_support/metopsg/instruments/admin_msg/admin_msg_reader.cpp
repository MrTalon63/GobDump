...
namespace metopsg
{
    namespace admin_msg
    {
        ...
        {
            // size() is unsigned, so size()-39 on a short packet became ~2^64, truncated to a huge
            // unsigned int for bzip2, which then read gigabytes past the payload looking for a stream.
            size_t insize = satdump::buf_remaining(packet.payload.size(), 39);
            if (insize == 0)
                return;

            unsigned int outsize = MAX_MSG_SIZE - 1; // Leave room for the NUL rapidxml needs below

            int ret = BZ2_bzBuffToBuffDecompress((char *)message_out, &outsize, (char *)&packet.payload[39], insize, 1, 1);
